...
class EnumUtils {
public:
  template <typename T> static constexpr auto toUnderlying(T t) noexcept {
    return static_cast<std::underlying_type_t<T>>(t);
  }

  template <typename EnumType>
  static String
  enumToString(EnumType enumValue,
               const std::map<EnumType, std::string> &enumToStrMap) {
    try {
  ...
  template <typename EnumType>
  static EnumType
  stringToEnum(const String &enumStr,
               const std::map<EnumType, std::string> &enumToStrMap) {
