...
namespace touka {
MonthView::MonthView(QWidget *parent) : QWidget(parent), ui(new Ui::MonthView) {
  ui->setupUi(this);
  ...
  connect(ui->btnPrevMonth, &QPushButton::clicked, this, [&] {
    const auto date =
        Date(curr_year_, MONTH_TO_INT_TOUKA(curr_month_), 1).addMonths(-1);
...
void MonthView::navigateTo(int year, Month month) {
  if (year == curr_year_ && month == curr_month_)
    return;
  curr_year_ = year;
  curr_month_ = month;
  refresh();
...
QString MonthView::getDateTitle() const {
  return QString("<html><head/><body><p><span style=\" font-size:32pt; "
                 "font-weight:700;\">"
                 "%1"
                 "</span><span style=\" font-size:32pt;\"> "
                 "%2"
                 "</span></p></body></html>")
      .arg(DateUtil::getLiteralMonth(curr_month_), QString::number(curr_year_));
}
} // namespace touka
