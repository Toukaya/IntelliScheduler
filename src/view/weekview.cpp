...
namespace touka {
WeekView::WeekView(QWidget *parent) : QWidget(parent), ui(new Ui::WeekView) {
  ui->setupUi(this);
...
void WeekView::initTable() {
  constexpr auto initEvtTable = [](QTableWidget *tableWidget, int rowHeight) {
    ...
    tableWidget->verticalHeader()->setDefaultAlignment(Qt::AlignTop |
                                                       Qt::AlignHCenter);
    tableWidget->verticalHeader()->setStyleSheet(
        "QHeaderView::section { color: #BDBDBD; background-color: #fff; "
        "border: node; padding-top: 4px; }");
  ...
  int centerLineY = currTime.hour() * 60 + currTime.minute() -
                    ui->twEvt->viewport()->height() / 2;
